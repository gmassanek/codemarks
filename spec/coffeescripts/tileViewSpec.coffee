describe 'Tile View', ->
  ...
  describe 'renders', ->
    it 'a %section.tile', ->
      renderSpy = spyOn(App.LinkCodemarkView.prototype, 'render')
      @view.render()
      expect(@view.$el.hasClass('tile')).toBeTruthy()
      expect(@view.$el.prop('tagName')).toBe('SECTION')

    it 'a LinkCodemarkView if the codemark exists', ->
  ...
  describe 'turnViewIntoForm', ->
    ...
    it 'replaces the tile with a view', ->
      renderSpy = spyOn(App.LinkFormView.prototype, 'render')
  ...
  describe 'listens to the view it created', ->
    it 'for when it wants to turn into a form', ->
      codemarkView = new App.LinkCodemarkView
        model: @codemark
